...
namespace Anamnese.API.Application.Services.Token
{
    public class TokenService : ITokenService
    {
        ...
        public string GenerateToken(string key, string issuer, string audience, ProfissionalModel user)
        {
            var claims = new[]
            {
            new Claim(ClaimTypes.Name, user.Username),
            new Claim(ClaimTypes.NameIdentifier, user.ProfissionalId.ToString())
